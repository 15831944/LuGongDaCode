...
class ConfigurationTableKeyNotFound : public ConfigurationTableError {
	...
	ConfigurationTableKeyNotFound(const std::string& wKey)
		:mKey(wKey)
	{ cout<<"WARNING -- cannot find configuration value " << mKey; }
};
...
class ConfigurationTable {
   ...
   char cfgfilename[256];
	...
	ConfigurationTable(const char* filename)
		{ assert(readFile(filename)); strcpy(cfgfilename,filename); }
	...
	*/
	...
	long long getValue(const std::string& key) const { return strtoll(getStr(key),NULL,10); }
	
	double getReal(const std::string& key) const { return strtod(getStr(key),NULL); }    
