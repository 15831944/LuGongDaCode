...
typedef std::vector<fcomplex> FcVector;
//@}


template<class Real>
inline Complex<Real> Complex<Real>::inv() const {
    Real nVal;
...
template<class Real>
Complex<Real> &Complex<Real>::operator+=(const Complex<Real> &a) {
    r += a.r;
    i += a.i;
    return *this;
}
...
template<class Real>
Complex<Real> &Complex<Real>::operator+=(Real a) {
    r += a;
    return *this;
}
...
template<class Real>
Complex<Real> Complex<Real>::unit() const {
    Real absVal = abs();
...
/// e^jphi
template<class Real>
Complex<Real> expj(Real phi) {
    return Complex<Real>(cos(phi), sin(phi));
}

/// phasor expression of a complex number
template<class Real>
Complex<Real> phasor(Real C, Real phi) {
    return (expj(phi) * C);
}

/// formatted stream output
template<class Real>
std::ostream &operator<<(std::ostream &os, const Complex<Real> &z) {
    os << "(" << z.r << ' ';
    //os << z.r << ", ";
    //if (z.i>=0) { os << "+"; }
    os << z.i << "j" << ")";
