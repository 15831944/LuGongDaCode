...
//16 bit parity with generator: D16 + D12 + D5 + 1,
static const unsigned long CRC16Generator = (1 << 16) + (1 << 12) + (1 << 5) + 1;
...
public:
    ...
    Generator(uint64_t wCoeff, unsigned wLen)
            : mCoeff(wCoeff), mState(0),
              mMask((1ULL << wLen) - 1),
              mLen(wLen), mLen_1(wLen - 1) { assert(wLen < 64); }
...
public:
    ...
    BitVector segment(size_t start, size_t span) {
        ...
        assert(wEnd <= mEnd);
        return BitVector(nullptr, wStart, wEnd);
    }

    BitVector alias() { return segment(0, size()); }
...
public:
    ...
    SoftVector(const Vector<float> &source1, const Vector<float> source2) : Vector<float>(source1, source2) {}
    ...
    */
    ...
    SoftVector segment(size_t start, size_t span) {
        ...
        assert(wEnd <= mEnd);
        return SoftVector(nullptr, wStart, wEnd);
    }

    SoftVector alias() { return segment(0, size()); }
