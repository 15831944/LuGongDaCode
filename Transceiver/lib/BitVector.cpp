...
void SoftVector::decode(ViterbiR2O4 &decoder, BitVector &target) const {
...

    {
        ...
        while (op < opt) {
            // Viterbi algorithm
            assert((signed) (match - matchCostTable) <
                   (signed) (sizeof(matchCostTable) / sizeof(matchCostTable[0]) - 1));
            assert((signed) (mismatch - mismatchCostTable) <
                   (signed) (sizeof(mismatchCostTable) / sizeof(mismatchCostTable[0]) - 1));
